...
document.getElementById('addRecebidoButton').addEventListener('click', function () {
    ...
    function formatNumber(value) {
        // Converte para string e divide em partes inteira e decimal
        const fixedValue = value.toString();
        const [integerPart, decimalPart] = fixedValue.split('.');
        
        // Se não houver parte decimal, adiciona '0000'
        const paddedDecimalPart = (decimalPart || '0000').padEnd(4, '0').slice(0, 4);
        
        return `R$ ${integerPart},${paddedDecimalPart}`;
    }
    ...
    const valorTotal = parseNumber(quantidadeRecebida) * parseNumber(valorUnitario);
