    ...
    function initPageScripts() {
        ...
        if (itemForm && ocForm && infoTable && notification && fornecedorSelect) {
            ...
            document.getElementById('addItemBtn').addEventListener('click', () => {
                ...
                const requiredFields = ['itemCodigo', 'itemDescricao', 'itemApresentacao'];
                const isFormValid = requiredFields.every(field => itemFormData.get(field));
            
                if (!isFormValid) {
                    showNotification('Os campos Código, Descrição e Apresentação são obrigatórios!');
...
document.getElementById("consultarb").onclick = function() {
...
document.getElementById("search-button").onclick = function() {
    ...
    if (input) {
        tbody.innerHTML += `<tr><td>001</td><td>Descrição para ${input}</td></tr>`;
        tbody.innerHTML += `<tr><td>002</td><td>Outra descrição para ${input}</td></tr>`;
    } else {
